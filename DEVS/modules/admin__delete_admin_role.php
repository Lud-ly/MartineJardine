...
/**
 * class Admin__delete_admin_role | file admin__delete_admin_role.php
 *
 ...
 * @subpackage admin__delete_admin_role
 * @author @AfpaLabTeam - Damien
 * @copyright  1920-2080 Afpa Lab Team - CDA 20303
 * @version v1.0
 */
class Admin__delete_admin_role	{
    ...
    private function main() {
        ...
        if (!isset($_POST['user_id'])) {
            $aResult['error'] = 'Id manquant';
        } else {
            $userId = $_POST['user_id'];
            if (($objet_service->getUserRole($userId) == 4) && ($objet_service->getConnectedUserRole() != 4)) {
                $aResult = ['error' => "Droits manquants : seul un SuperAdmin peut supprimer le rôle d'un SuperAdmin."];
            } else {
                $aResult = $objet_service->deleteAdminRole($userId);
            }
        }
